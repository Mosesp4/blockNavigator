...
app.get("/getblocknfo", async (req, res) => {
    try {
        const lastestBlock = await Moralis.EvmApi.block.getDateToBlock({
            date: Date.now(),
            chain: "0x1",
        })

        let blockNoOrParentHash = lastestBlock.toJSON().block;
        ...
        for(let i = 0; i < 5; i++) {
            ...
            blockNoOrParentHash = previousBlockNrs.toJSON().parent_hash;
        ...
        const response = {
            lastestBlock: lastestBlock.toJSON().block,
            previousBlockInfo
        };
...
// Endpoint for getting transactions for a specific address
